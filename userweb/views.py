...
class UserwebViews:
    ...
    def login(self):
        ...
        if "form.submitted" in request.params:
            ...
            with request.registry.settings["ldap.server"].connect() as conn:
                ...
                messages.append(("error", "Failed login"))
    ...
    def users(self):
        with self.request.registry.settings["ldap.server"].connect() as conn:
            ...
            if self.request.matched_route.name == "users":
                ...
                users = filter(
                    lambda u: u.uid != "nobody",
                    usermanagement.Group.Guests(conn).get_users(),
                )
                title = "Guests"
    ...
    def _system_add_user(
        self, uid, full_name, user_password, primary_group, secondary_groups
    ):
        settings = self.request.registry.settings
        with settings["ldap.server"].connect(
            settings["ldap.user"], settings["ldap.password"]
        ) as conn:
            ...
            new_user = usermanagement.User.add(
                conn, uid, full_name, user_password, primary_group, secondary_groups
            )

    @view_config(
        route_name="add_guest", renderer="templates/add_user.pt", permission="user"
    )
    @view_config(
        route_name="add_user", renderer="templates/add_user.pt", permission="admin"
    )
    def add_user(self):
        request = self.request
        with request.registry.settings["ldap.server"].connect() as conn:
            ...
            if "form.submitted" in request.params:
                ...
                if conn.User(
                    ...
                    messages.append(("success", "%(uid)s added" % {"uid": user_id}))
                    added = True
                else:
                    messages.append(("error", "Authentication Failed"))
        return {
            "title": title,
            "messages": messages,
            "groups": groups,
    ...
    @view_config(
        route_name="change_groups",
        renderer="templates/change_groups.pt",
        permission="admin",
    )
        ...
        with request.registry.settings["ldap.server"].connect() as conn:
            ...
            if "form.submitted" in request.params:
                if conn.User(
                    ...
                    secondary_groups = request.params.getall("user_secondary_groups")
                    user.update_groups(
                        primary=conn.Group(gid=primary_group),
                        secondary=[conn.Group(gid=x) for x in secondary_groups],
                    )
                    messages.append(
                        (
                            "success",
                            "%(uid)s's group membership changed" % {"uid": user_id},
                        )
                    )
                    changed = True
                else:
    ...
    @view_config(
        ...
        renderer="templates/change_pw.pt",
        permission="authed",
    )
    @view_config(
    ...
    @view_config(
        route_name="reset_computer_password",
        renderer="templates/change_pw.pt",
        permission="admin",
    )
    def change_pw(self):
        ...
        elif request.matched_route.name == "reset_computer_password":
            uid = request.matchdict["uid"]
            title = "Reset password for %(uid)s" % {"uid": uid}
            computer = True
        else:
        ...
        if "form.submitted" in request.params:
            with request.registry.settings["ldap.server"].connect() as conn:
                ...
                if new_pw != request.params["new_password"]:
                    ...
                    messages.append(
                        ("success", "Password for %(uid)s reset" % {"uid": uid})
                    )
                    changed = True
                else:
                    messages.append(
                        ("error", "Failed Authentication or Insufficent Permissions")
                    )
    ...
    @view_config(
        route_name="remove_user", renderer="templates/remove.pt", permission="user"
    )
    @view_config(
        route_name="remove_computer", renderer="templates/remove.pt", permission="admin"
    )
    def remove_user(self):
        ...
        object_type = (
            "computer"
            if self.request.matched_route.name == "remove_computer"
            else "user"
        )
        messages = []
        password = ""
            ...
            with request.registry.settings["ldap.server"].connect() as conn:
                ...
                if conf_uid != uid:
                    messages.append(
                        (
                            "error",
                            "UID entered does not match %(object_type)s to be deleted"
                    ...
                    messages.append(("success", "%(uid)s deleted" % {"uid": uid}))
                    deleted = True
                else:
                    messages.append(
                        ("error", "Failed Authentication or Insufficent Permissions")
        ...
        return {
            "title": "Are you sure you want to delete the %(object_type)s:  %(uid)s"
            % {"object_type": object_type, "uid": uid},
            "messages": messages,
            "url": remove_url,
            "confirm_uid": "",
            "object_type": object_type,
    ...
    @view_config(
        route_name="computers", renderer="templates/computers.pt", permission="admin"
    )
    def computers(self):
        with self.request.registry.settings["ldap.server"].connect() as conn:
    ...
    @view_config(
        route_name="add_computer",
        renderer="templates/add_computer.pt",
        permission="admin",
    )
        ...
        with request.registry.settings["ldap.server"].connect() as conn:
            ...
            if "form.submitted" in request.params:
                ...
                if conn.User(
                    ...
                    new_user = usermanagement.Computer.add(
                        conn, computer_id, computer_password
                    )
                    messages.append(("success", "%(uid)s added" % {"uid": computer_id}))
                    added = True
                else:
                    messages.append(("error", "Authentication Failed"))
        return {
